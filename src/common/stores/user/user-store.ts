...
interface IUserStore {
  ...
  user: User
  initUser: () => void
  login: (username: string, password: string, email: string) => Promise<User>
}
...
const userStore = create<IUserStore>((set) => ({
  ...
  login: async (username: string, userAvatar: string, email: string) => {
    const year = 67 - Number(username.slice(0, 2))
